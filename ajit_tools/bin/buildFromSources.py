...
def makeAa2C(system_name, aa_source_prefix, top_daemon_modules, use_gnu_pth):
    ...
    indent_cmd = "indent -orig aa2c/*"
    cmd_status = execSysCmd(indent_cmd)
    if(cmd_status != 0):
	logError("indent command failed: " + indent_cmd)
        ret_val = 1
...
def parseOptions(opts):
    ...
    sim_type = os.environ.get('AJIT_DEFAULT_VHDL_SIMULATOR')
    ...
    for option, parameter in opts:
	   ...
	   aa_source_prefix = parameter
           logInfo("aa-source-prefix = " + vhdl_library_name + ".")
        elif option ==  '-D':
           debug_flag = 1
    ...
    if (aa_source_prefix == None):
       aa_source_prefix = vhdl_library_name
...
def printUsage():
    print ''' 
     ...
     -S  <start-point> ({ "", to_llvm, to_aa, to_vc, to_vhdl, to_sim} ) 
     -E  <end-point> (one of {to_llvm, to_aa, to_vc, to_vhdl, to_sim} ) 
     -h  (print help message)
     -s  <simulator-type> (modelsim/ghdl)
     (-C  (c-source-dir))*
     (-I  (c-include-dir))*
...
def main():
    ...
    if sim_type == None:
       logError("simulator-choice not specified: set AJIT_DEFAULT_VHDL_SIMULATOR to modelsim or ghdl.")
       return 1
