...
def setGlobals(ajit_project_home, ajitcc):
	...
	SPARC_CC_FLAGS =  " -S -Wall -Werror -m32 -mcpu=v8 -ffreestanding -nostdlib -fschedule-insns -fschedule-insns2 -frename-registers "
	...
	#SPARC_LD_FLAGS = " -l gcc_s -L " + ajit_project_home + "/tools/cross-compiler/buildroot_cross_compiler/usr/sparc-buildroot-linux-uclibc/lib/ -e main -T "
	...
	SPARC_READELF_FLAGS=" --hex-dump=.text --hex-dump=.rodata --hex-dump=.rodata.str1.8 --hex-dump=.data "
...
def  compileFiles(work_area, src_files,src_dirs,include_dirs,define_strings,assembly_files, assembly_dirs, debug_mode, opt_level):
	...
	name,extn = os.path.splitext(sfilename)
        if(extn == ".c"):
           cc_command = SPARC_CC + " -c " + SPARC_CC_FLAGS  +  " -O" + str(opt_level) + " " + debug_mode + " " + include_string + define_string + " " + sfile  + " -o " + work_area + "/sparc-assembly/" + name + ".s"
...
def buildExecutable(work_area, obj_files, linker_script_file, elf_file, hex_dump_file,variable_dump_file,memmap_file, objdump_file):
    ...
    sparc_ld_command = SPARC_LD+SPARC_LD_FLAGS+ linker_script_file  + obj_file_string + " -o " + full_elf_file
    ret_val = execSysCmd(sparc_ld_command)
    if(ret_val != 0):
       logError("in building elf " + elf_file)
       return 1

    sparc_hexdump_command = SPARC_READELF+SPARC_READELF_FLAGS+ full_elf_file + " | tr -cd '\\11\\12\\15\\40-\\176' | grep 0x > "  +  work_area + "/" + hex_dump_file
...
def parseOptions(opts):
    ...
    return help_flag, work_area, linker_script_file, elf_file, hex_file, var_file, mmap_file, objdump_file, assembly_dirs, assembly_files,  src_dirs, src_files, include_dirs, define_strings , debug_mode, opt_level
...
def main():
    ...
    arg_list = sys.argv[1:]
    opts,args = getopt.getopt(arg_list,'W:L:C:c:S:s:I:D:E:H:V:M:O:hgo:')
    help_flag, work_area, linker_script_file, elf_file, hex_file, var_file, mmap_file, objdump_file, assembly_dirs, assembly_files,  src_dirs, src_files, include_dirs, define_strings, debug_mode, opt_level  = parseOptions(opts)
