...
def validate (fileName, choice, ignore_fpu, single_stepping_enabled, generate_detailed_trace, test_file_string, include_string, result_file, log_file, trace_file, path_test_file_fold, path_srch_file, define_string, enable_reg_write_trace, reg_write_trace):
	...
	if(single_stepping_enabled == '-s'):
		if(choice!='-A' and choice !='-V' and choice !='-F'):
	...
	if (choice == '-S') :
		    	print  "Note: executing " + path_proc_exec_aa_v3 + " -m " + mem_map_file + " -r " + result_file + " -l " + log_file + " " + trace_option + " " + trace_file + " "  + single_stepping_enabled 
		    	return_val = subprocess.call ([path_proc_exec_aa_v3, "-m", mem_map_file, "-r", result_file, "-l", log_file, trace_option, trace_file,single_stepping_enabled])
...
def main ():
	...
	arg_list = sys.argv[1:]
	opts,args = getopt.getopt(arg_list,'C,A,V,f,F,l,s,T,S')
