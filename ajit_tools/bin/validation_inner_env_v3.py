...
path_proc_exec_aa = validation_exec_location + "/ajit_core_sw_uarch_test"
...
#some mem_map xfiles contain a save instruction
...
def validate (fileName, choice, single_stepping_enabled, generate_detailed_trace, test_file_string, include_string, result_file, log_file, trace_file, path_test_file_fold, path_srch_file, define_string, enable_reg_write_trace, reg_write_trace):
	...
	#some mem_map xfiles contain a save instruction at the beginning (0xf0004000).
	#replace this with a nop
	...
	if (choice == '-C') :
		...
		proc_exec = path_proc_exec_C
        	if(enable_reg_write_trace):
		    	print  "Note: executing " + proc_exec + " -d -m " + mem_map_file + " -r " + result_file + " -l " + log_file + " " + trace_option + " " + trace_file + " -w " + C_reg_write_trace + " "
		    	return_val = subprocess.call ([proc_exec, "-d",  "-m", mem_map_file, "-r", result_file, "-l", log_file, trace_option, trace_file, "-w", C_reg_write_trace])
        	else:
