...
# parallel with validation_inner.py and text file and an option for 
...
def setGlobals(work_dir):
   ...
   # path of validation_parallel2.py
   path_validation_inner = ajit_project_home + "/tools/scripts/validation_inner.py"
   ...
   # give path of processor executable
   path_proc_exec_aa = validation_exec_location + "/ajit_cpu_uarch_test_doval"
   path_proc_exec_aa_v3 = ajit_project_home + "/processor/Aa_v3/Testbenches/bin/ajit_core_sw_doval_uarch_test"
   ...
   path_proc_exec_FPGA = validation_exec_location + "/ajit_cpu_uarch_test_doval_riffa_riffa"
...
def help_sec ():
	...
	print " -----------------------------------------------------------------------------------------------------------"
	print " USAGE:"
	print "        validation_outer.py [MODEL] [OPTIONS] [-t TIMEOUT_IN_SECONDS] [-j JOBS_TO_RUN_IN_PARALLEL]  PATHS..."
	print " -----------------------------------------------------------------------------------------------------------"
	print " MODEL:"
	print "      -C : C executable"
	print "      -A : Aa executable"
	print "      -S : Aa_v3 executable"
	print "      -V : VHDL model"
	print "      -F : FPGA model"
	print " ------------------------"
	print " OPTIONS:"
	...
	print " ------------------------"
	print " CLEANUP:"
	print "      validation_outer.py -c PATHS..."
...
def main ():
	...
	# options
	arg_list = sys.argv[1:]
	opts,args = getopt.getopt(arg_list,'j:t:c,h,A,C,V,f,F,l,s,T,S')
	...
	if choice != "-C" and choice != "-A" and choice != "-V" and choice != "-F" and choice != "-S":
		print "Error : Provide the executable option ie '-A' or '-C' or '-V' or '-F' or '-S'"
		return 1
			
	if jobs == "" :
		print "Error in Usage :"
		print "Use as - validation_outer.py -choice -t <timeout> -j <parallel_jobs> <path1> <path2> ...\n"
		return 1
	...
	if not os.path.exists (path_proc_exec_aa_v3) and choice == '-S':
		print path_proc_exec_aa_v3
		print "Error : terminating validation, cannot find processor executable"
		return 1
