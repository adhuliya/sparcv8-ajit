...
% resLockStartAddr = confObj.locks.resLocksStartAddr

! Usage Note:
! __cortos_lock_acquire_buzy(<index: an-integer-index>);
!   CRITICAL_SECTION_CODE...
! __cortos_lock_release(<index: an-integer-index>);
! ----or-------or--------
! status = __cortos_lock_acquire(<index: an-integer-index>);
! if (status == 1) {
!   CRITICAL_SECTION_CODE...
!   __cortos_lock_release(<index: an-integer-index>);
! }

.global __cortos_lock_acquire_buzy
.global __cortos_lock_acquire
.global __cortos_lock_release

__cortos_lock_acquire_buzy:
  ! i0 contains an index to the correct locking variable
  save  %sp, -96, %sp       ! func prefix

  set {{ resLockStartAddr }}, %l0
