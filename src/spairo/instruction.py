...
class Instruction():
  ...
  def __init__(self, asmChunk=None, name=None, mnemonic=None, suffix=None, latency=None, regRead=None, regMod=None,
               resUsed=None, destLabel=None, delaySlot=None):
  ...
  def parse(self):
    ...
    if self.isUnsafeBranchInstr():
      print("ERROR : Unsafe instruction : '{}'".format(self.asmChunk.text), file=sys.stderr)
      exit(1)
  ...
  def isUnsafeBranchInstr(self):
    """
    (invoke after the instruction parsing is complete.)
    unsafe branch instructions.
    If a branch instruction uses a register value as branch address, then it could be unsafe.
    :return:
    """
    unsafe = False
    if self.mnemonic in sparc.labeledBranch:
      #log.info("Possibly Unsafe instruction: %s, %s", self.asmChunk.text, self.destLabel)
      if self.destLabel is None or len(self.destLabel) == 0:
        log.error("Unsafe instruction: %s", self.asmChunk.text)
        unsafe = True
    return unsafe
  ...
  def __str__(self):
    string = """\
        ...
        """
    return dedent(string).format(self.asmChunk, self.name, self.mnemonic, self.suffix, self.regRead, self.regMod,
                                 self.resUsed, self.latency, self.destLabel)
